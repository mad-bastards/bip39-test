(function() {
    ...
    function TableRow(index, isLast) {
        ...
        function calculateValues() {
            setTimeout(function() {
                ...
                let indexText = getDerivationPath() + "/" + index;
                if (useHardenedAddresses) {
                    indexText = indexText + "'";
                }
                ...
                if (isSegwit) {
                    ...
                    if (isP2wpkh) {
                        ...
                        // This is a 1-of-1
                        var witnessScript = libs.bitcoin.script.multisig.output.encode(1, [key.getPublicKeyBuffer()]);
                        const redeemScript = libs.bitcoin.script.witnessScriptHash.output.encode(libs.bitcoin.crypto.sha256(witnessScript));
    ...
    function showMore() {
        ...
        let start = parseInt(DOM.moreRowsStartIndex.val());
        if (isNaN(start)) {
    ...
    function getLanguage() {
        ...
        let language = getLanguageFromPhrase();
    ...
    function writeSplitPhrase(phrase) {
        ...
        const group = [[], [], []];
        let groupI = -1;
        let seed = Math.abs(sjcl.hash.sha256.hash(phrase)[0]) % 2147483647;
        while (left.length>0) {
            groupI=(groupI+1)%3;
    ...
    function setMnemonicFromEntropy() {
        ...
        let mnemonicLength = DOM.entropyMnemonicLength.val();
    ...
    function getEntropyTypeStr(entropy) {
        ...
        if (entropy.base.asInt === 52) {
            ...
            // Detect full deck
            const uniqueCards = [];
            for (let uniqueCard in dupeTracker) {
    ...
    function updateCsv() {
        let tableCsv = "path,address,public key,private key\n";
    ...
    function stripHexPrefix(address) {
        ...
        const hasPrefix = (address.substring(0, 2) === "0x" ||
    ...
    function toChecksumAddressForRsk(address, chainId = null) {
        ...
        const keccakHash = libs.ethUtil.keccak256(prefix + stripAddress)
            .toString("hex")
            .replace(/^0x/i, "");
